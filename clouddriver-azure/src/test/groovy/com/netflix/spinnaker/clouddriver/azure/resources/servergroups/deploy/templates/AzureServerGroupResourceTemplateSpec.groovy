...
class AzureServerGroupResourceTemplateSpec extends Specification {
  ...
  def 'generate server group template with extensions profile'() {
    description = createCustomDescription()
    String template = AzureServerGroupResourceTemplate.getTemplate(description)

    expect:
    template.replaceAll('"createdTime" : "\\d+"', '"createdTime" : "1234567890"') == expectedCustomScriptTemplate
  }
  ...
  private static AzureServerGroupDescription createCustomDescription() {
  ...
  private static String expectedCustomScriptTemplate = '''{
  "$schema" : "https://schema.management.azure.com/schemas/2015-01-01/deploymentTemplate.json#",
  "contentVersion" : "1.0.0.0",
  "parameters" : {
    "location" : {
